...
const SECURITY_NOTICE = {
  ...
  en: {
    ...
    hide: 'Do not show again today', close: 'Close',
    sections: [
  ...
  es: {
    ...
    hide: 'No volver a mostrar hoy', close: 'Cerrar',
    sections: [
...
function reportCsv(rows){
  ...
  return data.map(row=>row.map(v=>`"${String(v==null?'':v).replace(/"/g,'""')}"`).join(',')).join('\n');
}
...
async function uploadEncryptedAttachment(id,file) {
  ...
  try {
    ...
    m.files=m.files||[];
    const versionGroup=String(file.name).normalize('NFKC').toLocaleLowerCase();
    ...
    m.files.push({name:file.name,type:file.type,size:file.size,storagePath,encrypted:'lcb-e2ee-v1',uploadedBy:currentUser().id,uploadedAt:Date.now(),versionGroup,version});
...
function render() {
  ...
  else if (route.startsWith('/matters/')) content = viewMatter(route.split('/')[2],route.split('/')[3]);
  else if (route.startsWith('/matters')) content = viewMatters();
  else if (route.startsWith('/search')) content = viewGlobalSearch();
  else if (route.startsWith('/weekly')) content = viewWeekly();
  else if (route.startsWith('/calendar')) content = viewCalendar();
  else if (route.startsWith('/clients')) content = viewClients();
  else if (route.startsWith('/followups')) content = viewFollowups();
  else if (route.startsWith('/team')) content = viewTeamWorkload();
  else if (route.startsWith('/deadline')) content = viewDeadlineCalculator();
  else if (route.startsWith('/reports')) content = viewReports();
...
async function clientImportRows(file) {
  ...
  return lines.map(line=>Object.fromEntries(parse(line).map((v,i)=>[heads[i],v])));
}
...
document.addEventListener('click', async ev => {
  ...
  switch (action) {
    ...
    case 'mobile-nav-link':
      ...
      setTimeout(()=>go(el.getAttribute('href')), 260);
      break;
    case 'open-tutorial':
      setMobileNavOpen(false);
...
document.addEventListener('submit', async ev => {
  ...
  if (action === 'login') {
    ...
    session = { userId: user.id }; lastUserActivityAt = Date.now(); state.bulkSelected.clear(); state.clientSelected.clear(); state.trashSelected.clear(); state.notificationSelected.clear();
